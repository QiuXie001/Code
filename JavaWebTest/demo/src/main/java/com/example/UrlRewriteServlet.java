...
public class UrlRewriteServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (uri!=null&&uri!="") {
            ...
            request.getRequestDispatcher("/LoginUrlRewrite.jsp").forward(request, response);
        }
        else {
            ...
            response.setContentType("text/html;charset=UTF-8");
            // 将获取的数据封装成对象，存储在request对象中,request对象中可以存储任何数据类型（
            response.setHeader("Refresh", "0;URL=/demo/UrlRewrite?username="+ username.toString() + "&password=" + password.toString());
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (LoginCheck) {
            ...
            response.setHeader("Refresh",
                    "5;URL=/demo/WorkManage/shows?username=" + username + "&password=" + password);
        } else {
            response.setContentType("text/html;charset=UTF-8");
            PrintWriter out = response.getWriter();
            ...
            out.println("</body></html>");
            response.setHeader("Refresh", "5;URL=/demo/UrlRewrite");
        }
    }
